...
def setup_context_menu(graph):
    ...
    edit_menu.add_command('New Node', _create_new_node,'N')
    edit_menu.add_command('Add Input', _add_input, 'I')
    edit_menu.add_separator()
...
def _create_new_node(graph):
    node = graph.create_node('com.chantasticvfx.{}'.format('Blank'),
                             name ='blank' )

    input0 = QtGui.QInputDialog.get_input(self, 'add name', 'enter connection name')
    print(input0)



def _add_input(graph):
    from ..widgets import input


    app = QApplication(sys.argv)
    dlgMain = input.InputDialog()
    #dlgMain.graph = graph
    dlgMain.show()
    app.exec_()
